import { Avatar, Box, Chip, LinearProgress } from "@material-ui/core";
import {
  CheckCircleOutlined,
  HighlightOffOutlined,
  RadioButtonUncheckedOutlined,
} from "@material-ui/icons";
import React, { useEffect, useState } from "react";

import { getTestResultByResultId } from "../../../services/tests";
import { CustomDialog } from "../../../shared/components/CustomDialog";
import { LoadingScreen } from "../../../shared/components/LoadingScreen";
import { errorNotification } from "../../../utils/notifications";

export const ViewResultDialog = ({
  currentResult,
  visible,
  handleClose,
  testId,
}) => {
  const [loading, setLoading] = useState(null);
  const [result, setResult] = useState(null);

  useEffect(() => {
    if (visible && testId && currentResult) {
      fetchResult();
    }
  }, [visible, testId, currentResult]); // eslint-disable-line

  const fetchResult = async () => {
    try {
      setResult(null);
      setLoading(true);
      const { data } = await getTestResultByResultId(testId, currentResult.id);
      setResult(data);
      setLoading(false);
    } catch (error) {
      errorNotification(error);
      setLoading(false);
    }
  };

  const getOptions = (options) => {
    return (
      <>
        {options.map((option) => {
          const icon =
            option.selected && option.isRight ? (
              <CheckCircleOutlined style={{ color: "green" }} />
            ) : option.isRight ? (
              <RadioButtonUncheckedOutlined style={{ color: "green" }} />
            ) : option.selected ? (
              <HighlightOffOutlined style={{ color: "red" }} />
            ) : (
              <RadioButtonUncheckedOutlined style={{ color: "lightgray" }} />
            );
          return (
            <Box key={option.id} pb={1} display="flex" alignItems="center">
              <Box minWidth="30px" display="flex" alignItems="center">
                {icon}
              </Box>
              <Box className="single-option">{option.option}</Box>
            </Box>
          );
        })}
      </>
    );
  };

  const totalQuestions = result && result.length;
  const totalCorrectAnswers =
    result &&
    result.filter((r) => r.options.some((o) => o.selected && o.isRight)).length;

  if (!visible) {
    return null;
  }

  return (
    <>
      <CustomDialog
        title={
          <Box display="flex">
            Result
            <Box
              component={Chip}
              avatar={<Avatar />}
              label={currentResult.username}
              ml={2}
            />
            {!loading && result && (
              <Box width="150px" ml={2}>
                <Box textAlign="center" fontWeight="normal" fontSize="14px">
                  {totalCorrectAnswers} / {totalQuestions}
                </Box>
                <Box>
                  <LinearProgress
                    variant="determinate"
                    value={(totalCorrectAnswers / totalQuestions) * 100}
                  />
                </Box>
              </Box>
            )}
          </Box>
        }
        visible={visible}
        maxWidth="md"
        showCloseIcon
        onClose={handleClose}
        scroll="body"
      >
        {loading ? (
          <Box mb={3}>
            <LoadingScreen />
          </Box>
        ) : (
          <>
            {result && result.length ? (
              <Box mb={3}>
                {result.map((question, index) => (
                  <Box key={index} className="questions-wrapper">
                    <Box display="flex" alignItems="center">
                      <Box className="question-index">{index + 1}</Box>
                      <Box>{question.question}</Box>
                    </Box>
                    {question.options && (
                      <Box ml={4.25} mt={2}>
                        {getOptions(question.options)}
                      </Box>
                    )}
                  </Box>
                ))}
              </Box>
            ) : null}
          </>
        )}
      </CustomDialog>
      <style jsx global>{`
        .questions-wrapper {
          margin-top: 10px;
        }
        .questions-wrapper .question-index {
          background: rgb(5, 83, 165);
          color: #fff;
          font-weight: bold;
          display: flex;
          justify-content: center;
          align-items: center;
          width: 30px;
          height: 30px;
          border-radius: 4px;
          margin-right: 10px;
        }
        .questions-wrapper .single-option {
          padding: 5px 10px;
          border-radius: 10px;
          background: #9e9e9e2e;
        }
      `}</style>
    </>
  );
};
